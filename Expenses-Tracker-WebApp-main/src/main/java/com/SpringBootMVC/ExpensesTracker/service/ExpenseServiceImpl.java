...
public class ExpenseServiceImpl implements ExpenseService {
    ...
    public void update(ExpenseDTO expenseDTO) {
        ...
        if (existingExpense == null) {
            throw new RuntimeException("Expense not found with id: " + expenseDTO.getExpenseId());
        }
    ...
    public List<Expense> findFilterResult(FilterDTO filter) {
        ...
        if (!"all".equals(filter.getCategory())) {
            ...
            Category catObj = categoryService.findCategoryByName(category);
            if (catObj == null) {
                throw new RuntimeException("Category not found: " + category);
            }
            int categoryId = catObj.getId();
